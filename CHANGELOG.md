...
- **Migration-time parity/performance spot-checks (not retained)**: during development, throwaway scripts diffed the new Rust `topos inspect --json` against the last published Python release (`topos-mcp==0.3.11`) over a small fixture corpus and timed a handful of invocations — an informal sanity check that the rewrite is a behavior-preserving, faster drop-in, not a rigorous benchmark. Those measurements were single-run and machine-specific, and the scripts and corpus were removed rather than kept as evergreen tooling. Formal parity and performance validation will be run against the shipped v0.4.0 release; treat any migration-era figures as indicative only.
