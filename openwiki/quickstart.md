...
- `topos install`, `topos uninstall`, and `topos status` manage only Topos-owned MCP registrations in nine agent harnesses; see [agent-harness registration](workflows/harness-registration.md).
...
| Change MCP project rollup, file ranking, paging, or named worst-file lists | [Project evaluation ranking](workflows/agent-and-cli.md#project-evaluation-ranking) | `topos/mcp/src/tools/evaluate.rs` | `topos_evaluate_project`, `build_project_result`, `RowKeys` | `ranking_lists_evaluate_gates_once_per_row`; ranking-list tests in `evaluate.rs` | `cargo test -p topos-mcp ranking_lists_evaluate_gates_once_per_row` |
...
| Add or change agent-harness registration or its status report | [Agent-harness registration](workflows/harness-registration.md) | `topos/cli/src/commands/install/{harness.rs,status.rs,artifact.rs}` | `HARNESSES`, `HarnessSpec`, `Artifact`, `State`, `harness_json` | `topos/cli/tests/install_e2e.rs` | `cargo test -p topos --test install_e2e` |
