...
For release changes, inspect the complete workflow rather than assuming a generic Python publish: it handles platform matrix artifacts, optional macOS signing/notarization, extension packaging, Homebrew formula publication, and trusted publishing. Exercise `tests/packaging/test_install_sh_preflight.py` when changing the formula template, installer coexistence checks, or Homebrew install guidance; its channel-detection cases must continue to distinguish a declared Homebrew prefix from an ordinary local executable, including paths under Intel `/usr/local`. Never read or record secret values; workflow secret identifiers are sufficient for operational reasoning.
