...
impl ProgramDependenceGraph {
    ...
    pub fn from_uast(uast_root: &UASTNode) -> Self {
        ...
        let mut edges = compute_data_dependence(&statements);
...
fn compute_data_dependence(statements: &[UASTNode]) -> Vec<DependenceEdge> {
    let mut edges = Vec::new();
    let mut last_def: HashMap<String, String> = HashMap::new();

    for stmt in statements {
        let (defs, uses) = defs_and_uses(stmt);
...
fn defs_and_uses(stmt: &UASTNode) -> (HashSet<String>, HashSet<String>) {
    let mut defs = HashSet::new();
    let mut uses = HashSet::new();

    fn walk(node: &UASTNode, in_lhs: bool, defs: &mut HashSet<String>, uses: &mut HashSet<String>) {
        if node.kind == "AssignExpr" {
            let mut children = node.children.iter();
            if let Some(lhs) = children.next() {
                walk(lhs, true, defs, uses);
                for rhs in children {
                    walk(rhs, false, defs, uses);
                }
            }
            return;
        }
        ...
        for child in &node.children {
            walk(child, in_lhs, defs, uses);
        }
    }

    walk(stmt, false, &mut defs, &mut uses);
    (defs, uses)
}
...
fn identifier_name(node: &UASTNode) -> String {
