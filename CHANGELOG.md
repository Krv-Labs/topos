...
- **`mdg.instability` and `mdg.main_sequence_distance` are now advisory** (`gates_achieved: false`), joining `cfg.cyclomatic`. COMPOSABLE gates on `mdg.fan_in` / `mdg.fan_out` only. Instability is a ratio whose resolution is `1 / (Ca + Ce)`, and at file granularity that denominator is a single digit: the share of attainable readings `{k/n}` that land inside the calibrated band swings with `n` (33% at `n = 2`, 50% at `n = 3`, 20% at `n = 4`) rather than with design quality, and over Topos's own 176 files the pass rate tracked that grid density. Both metrics are still scored, interpreted, and offered as refactor targets at severity `improve` — they simply cannot hard-fail a file. Reported scores are unchanged (`score` is `min` over all gates, advisory included). Gating coupling properly needs module granularity; see `docs/decisions/composable-at-module-granularity.md`.
