...
- **Evaluate summary floor and COMPOSABLE notices** — SLOP floor is `X  SLOP · N% average` (no ❌ medal + duplicate lattice name). GitNexus setup warnings render inside the summary card with the install orange `↻` glyph instead of a bare `gitnexus:` stderr banner; recoverable misses tip `topos depgraph generate`. JSON `warnings` stay full machine strings.
