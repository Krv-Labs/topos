...
//! interactively. Calls straight into the same `topos-core` functions the
//! MCP tools use (`crates/topos-mcp/src/tools/{graphify,refactor}.rs`) — no
//! logic duplicated between CLI and MCP.

use std::path::PathBuf;

use clap::{Args, Subcommand};
use topos_core::adapters::graphify::{ensure_graphify_graph, GRAPHIFY_GRAPH_FILE};
use topos_core::functors::probes::graphify::orphans::{
    calculate_graphify_orphans, FragileEdge, GraphifyOrphanResult, OrphanNode,
};
use topos_core::graphs::graphify::GraphifyGraph;
...
fn run_generate(args: GenerateArgs) -> Result<(), String> {
    ...
    let graph_file =
        topos_core::adapters::graphify::graphify_out_dir(&target_dir).join(GRAPHIFY_GRAPH_FILE);
