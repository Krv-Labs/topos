...
class DeclarationCoverageReport:
    ...
    declaration_coverage_rate: float
    ...
    uncovered_declarations: tuple[tuple[str, float], ...]
    include_unknown: bool
...
def declaration_coverage(
    ...
    *,
    ...
    Args:
        ...
        coverage_threshold: Best-match recall at or above which a PUT
            declaration is considered covered. Default: 0.5 (a test body must
            share at least half the PUT declaration's kind multiset to count as
            covering it — loose enough to tolerate idiomatic test scaffolding,
            strict enough to reject wholly unrelated functions). Not empirically
            calibrated; treat as a starting point pending validation data.
    ...
    # Test → PUT reverse bipartite (precision signal)
    ...
    # F2 score: beta=2 emphasizes recall 4x more than precision
    r, p = mean_decl_cov, mean_test_prec
    f2 = 5.0 * p * r / (4.0 * p + r) if (4.0 * p + r) > 0.0 else 0.0
    ...
    return DeclarationCoverageReport(
        ...
        declaration_coverage_rate=decl_cov_rate,
        ...
        uncovered_declarations=tuple(uncovered),
        include_unknown=include_unknown,
    )
