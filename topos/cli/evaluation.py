...
def _format_file_row(rank_str: str, filepath: str, result: dict[str, object], max_file_len: int = 42) -> str:
    ...
    if isinstance(scores, dict):
        ...
        for p in ordered_pillars:
            if p in scores:
                ...
                pillar_parts.append(f"{label_str}{score_percent_str} {bracket_left}{gauge}{bracket_right}")
...
def _output_file_details(results: list[dict[str, object]], verbose: bool) -> None:
    for result in results:
        ...
        avg_score = _display_score(result)
        click.echo(f"  {result['file']}  {medal_styled}  {click.style(f'{avg_score:.0f}%', bold=True)}")
        ...
        if isinstance(scores, dict):
            ...
            for dim in ["simple", "composable", "secure"]:
                if dim in scores:
                    ...
                    status_styled = click.style(f"{status:<4}", fg="green" if status == "PASS" else "red")
                    gauge = _build_gauge(score, threshold, width=20)
                    
                    dim_label = f"    {dim:<10}"
                    bracket_l = click.style("[", dim=True)
                    bracket_r = click.style("]", dim=True)
                    score_str = click.style(f"{score:>3.0f}%", fg="green" if status == "PASS" else "red")
                    req_str = click.style(f"(Req: {threshold:.0f}%)", dim=True)
                    
                    click.echo(f"{dim_label} {bracket_l}{gauge}{bracket_r}  {score_str}  {req_str}  {status_styled}")
...
def output_text(results: list[dict[str, object]], verbose: bool) -> None:
    ...
    click.echo(click.style(f"Evaluated {len(results)} file{'s' if len(results) != 1 else ''}", bold=True))
    ...
    if verbose or len(results) <= _DETAIL_FILE_LIMIT:
        ...
        if pillars:
            ...
            for pillar in _pillar_order(pillars):
                ...
                status_styled = click.style(f"{status_text:<8}", fg="green" if is_passing else "red", bold=True)
    ...
    click.echo()
    click.echo(f"  Directory Average Score: {click.style(f'{average:.0f}%', bold=True)} (Mean across all files)")
    ...
    meet_styled = click.style(f"{meet_val.symbol} {meet_val.name}", fg="green" if meet_val == EvaluationValue.IDEAL else "red", bold=True)
    click.echo(f"  Directory Floor Verdict: {meet_styled} (Pointwise lattice meet)")
    click.echo(click.style(f"  [INFO] The Floor Verdict is the minimum category-theoretic level achieved across all files.", dim=True))
