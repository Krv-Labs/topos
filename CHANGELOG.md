...
- **`topos evaluate` scores COMPOSABLE by default**: the CLI now detects whether `gitnexus` is on `$PATH` and whether `.gitnexus` is present and fresh (reusing the same `depgraph_status` staleness classifier the MCP server already used), regenerating the dependency graph via `gitnexus analyze --skip-agents-md` when it's missing or stale, then attaching the resulting MDG so COMPOSABLE scores alongside SIMPLE/SECURE with no extra flags. Any failure in that chain — GitNexus not installed, generation failing, a schema mismatch — degrades gracefully to SIMPLE/SECURE-only with a `stderr` notice rather than failing the run. New flags: `--no-composable` (skip GitNexus entirely) and `--gitnexus-dir <dir>` (override the default `<cwd>/.gitnexus`).
...
- **Drop-in parity and performance harness, validated then removed**: during the migration, `scripts/parity_check.py` and `scripts/benchmark_perf.py` shelled out to both the new Rust `topos` binary and the last published Python release (`topos-mcp==0.3.11`) over a small six-language fixture corpus (`parity/corpus/`), diffing `topos inspect --json` output and timing invocations to confirm the rewrite is a behavior-preserving, faster drop-in replacement — not a behavior change. Last validated run: **12/12 corpus files matched** (9 exact matches, 3 allowlisted divergences on `ast.max_function_complexity` — Python, issue [#153](https://github.com/Krv-Labs/topos/issues/153), plus Go and Rust from the per-arm `match`/`switch` counting fix above — all documented correctness improvements over the old buggy behavior, not regressions). Benchmarks (this branch vs. published `topos-mcp==0.3.11`, M-series Mac):
  ...
  Per-invocation is the number that matters for Topos's primary use case — an agent calling the CLI once per edited file (`topos_assess_worktree_change`, `topos_evaluate_file`, pre-commit hooks) — and the gap is structural (compiled binary vs. Python interpreter startup), expected to hold across machines. The harness itself (`parity/`, `docs/parity.md`, the two scripts) was a migration-verification artifact, not evergreen tooling, and was removed after this validation rather than kept in the repository long-term.
