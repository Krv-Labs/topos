...
1. **`UserPreferences`** — a strict total order over **all four** generators, e.g. `[COMPOSABLE, SECURE, SIMPLE, NAVIGABLE]`:
   ...
   - Default ranking is `(SIMPLE, COMPOSABLE, SECURE, NAVIGABLE)`. NAVIGABLE ranks **last** deliberately while its threshold is `PROVISIONAL`, so it is the first thing a relaxation walk concedes.
