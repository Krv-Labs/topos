...
- **`pi` agent harness support** ([#345](https://github.com/Krv-Labs/topos/issues/345), [#355](https://github.com/Krv-Labs/topos/pull/355)) — `topos install`, `topos status`, and `topos uninstall` now support the `pi` agent harness, configuring `~/.pi/agent/settings.json`.
- **Built-in ignore defaults and `.toposignore`** ([#335](https://github.com/Krv-Labs/topos/issues/335), [#353](https://github.com/Krv-Labs/topos/pull/353)) — discovery automatically skips minified assets (`*.min.*`), non-source folders (`fixtures`, `vendor`, `testdata`, `__snapshots__`), and generated files carrying `@generated` / `Code generated by ... DO NOT EDIT` header markers; adds a repository `.toposignore` covering test fixtures and benchmarks.

### Fixed

- **Graceful handling for 0 supported source files** ([#352](https://github.com/Krv-Labs/topos/issues/352), [#353](https://github.com/Krv-Labs/topos/pull/353)) — evaluating directories with 0 supported source files (such as type-only or configuration packages) now emits an informational notice and exits 0 instead of returning a fatal exit code.
- **Scoping ambiguous skip directory names** ([#334](https://github.com/Krv-Labs/topos/issues/334), [#353](https://github.com/Krv-Labs/topos/pull/353)) — ambiguous skip names (`coverage`, `build`, `out`, `dist`, `env`) are scoped to top-level/root directory traversal only, so legitimate source modules like `src/commands/coverage/` or `src/build/` are scanned.
- **TypeScript generic type imports and dynamic subscript accesses** ([#347](https://github.com/Krv-Labs/topos/issues/347), [#348](https://github.com/Krv-Labs/topos/issues/348), [#354](https://github.com/Krv-Labs/topos/pull/354)) — inline dynamic type imports in TypeScript generic arguments (`load<typeof import("...")>()`, `load<import("...")>()`) are sanitized during AST dispatch so tree-sitter parses the call expression cleanly without misidentifying `<` as a binary comparison operator and failing parse validation. In addition, `subscript_expression` is mapped to `MemberExpr` in the JavaScript/TypeScript UAST mapper, preventing defensive bracket lookups (`obj[key]`) from dropping to `Unknown` or being misclassified as dangerous execution call sinks.
- **Deterministic JSON serialization** ([#332](https://github.com/Krv-Labs/topos/issues/332), [#355](https://github.com/Krv-Labs/topos/pull/355)) — `ClassificationResult` maps (`dimensions`, `scores`, `raw_metrics`, `interpretation`) and `combine_dimensions` use `BTreeMap`, making CLI `--json` output byte-deterministic and alphabetical across runs regardless of `serde_json` feature unification.
- **Status clarity for low scores** ([#350](https://github.com/Krv-Labs/topos/issues/350), [#355](https://github.com/Krv-Labs/topos/pull/355)) — summary table displays `! WARN` instead of `✓ PASS` when a passing pillar has a diagnostic score below 25%.
- **MCP protocol negotiation** ([#337](https://github.com/Krv-Labs/topos/pull/337)) — keep every known MCP revision negotiable under `rmcp` 3.
