...
`topos/cli/src/commands/install/harness.rs` is the canonical eight-row `HARNESSES` table: Claude Code, Claude Desktop, Codex CLI, Gemini CLI, GitHub Copilot CLI, Cursor, VS Code, and Google Antigravity. A harness owns one artifact—the `topos` MCP-server entry in its user-scope config—not a bundle of unrelated files. Add a harness by extending that table and the supported artifact/path behavior rather than creating parallel target lists.
...
- Plain JSON `mcpServers.topos` for Claude, Gemini, Copilot, Cursor, and Antigravity.
...
1. Start in `harness.rs`: define the ID, config path, artifact format, detection predicate, and any explicit caveat. Keep IDs unique and config paths distinct.
...
4. Add unit tests beside the affected module and an end-to-end case in `topos/cli/tests/install_e2e.rs` for a CLI-visible lifecycle property.
5. Run `cargo test -p topos --test install_e2e`. This is the consumer-facing test: it drives the real binary against a scratch `$HOME`, checks `status --json`, preserves foreign content, and snapshots both files and directories before/after the round trip.
