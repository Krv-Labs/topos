...
- **pi also gets the skill route, which works without any extension** ([#345](https://github.com/Krv-Labs/topos/issues/345)) — because an MCP entry alone configures nothing on pi, `topos install pi` additionally references an already-installed topos skill directory from `settings.json`'s `skills` array. It is the one harness with two artifacts, and topos still never writes skill content: it appends one directory path, and skills stay ClawHub / Hermes / openclaw's. Three outcomes — the skill is already on pi's default scan path (nothing written, reported as `discovered`), it lives in `~/.agents/skills` or `~/.claude/skills` (referenced), or it isn't installed (nothing written, with the `openclaw skills install` command). `topos status --json` reports this as a `skillRef` object on the pi row, never folded into the harness verdict, and `topos uninstall pi` removes the path and drops an emptied `skills` key.
