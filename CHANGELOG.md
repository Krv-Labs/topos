...
- **`topos_inspect_code` now scores COMPOSABLE (wire-visible, closes #216).** It never resolved a dependency graph, so it reported `coupling_available: false` and a verdict up to one medal below `topos_evaluate_file` and the `topos inspect` CLI on the same file. It now takes `gitnexus_dir` and `no_composable` and resolves the graph through the same shared helpers as `topos_evaluate_file`, so the three agree. The `code`-string form is unchanged: with no file there is no module in the graph, so it still reaches SIMPLE/SECURE only.
- **`refactor_targets[0]` is the top gating target when no pillar preference is given (wire-visible).** The gating tier previously sat behind pillar rank, so with no `preferences.ranking` the internal default order (simple, secure, composable) could still put an advisory `cfg.cyclomatic` ahead of a real gate failure in another pillar. With no ranking supplied the tier now leads; when `preferences.ranking` *is* supplied the stated pillar order still wins and can legitimately put an advisory target first. Target contents, severities, scores, and verdicts are unaffected — only the array order for the no-ranking case.
- **`topos_generate_depgraph` caps GitNexus child output.** `message`, `error`, and the markdown channel carried the subprocess's entire output verbatim — kilobytes of Node stack trace on failure, and progress chatter on *success*, since the adapter folds captured stdout into the message too. All three are now bounded by the same 200-byte cap with an explicit elision marker. `blocked_by` / `risk_flags` codes are unchanged, and short messages pass through untouched. The `topos` CLI still prints GitNexus output in full on stderr.
- **MCP evaluation responses default to a compact shape (wire-visible).** `structured_content` was carrying, per file row, the full raw-metric map plus a block of always-null overlay fields and always-empty vectors. Three changes, all in the `topos-mcp` output models:
  - `verbose` now defaults to *off* internally, not just at the tool boundary. `topos_evaluate_code`, `topos_evaluate_file`, `topos_evaluate_project`, and `topos_inspect_code` are unchanged for callers — `verbose: true` restores `raw_metrics` and the untrimmed `interpretation` map, including on project rows, which the structured channel used to emit even at `verbose: false` (the markdown channel already honored the flag; the two now agree). It does *not* restore the null/empty fields dropped by the next bullet, which is unconditional. The exception is `topos_assess_*`, which has no `verbose` parameter and never set one: its `current` / `proposed` evaluations now always omit `raw_metrics` and trim `interpretation` to failing generators, with no way to restore either. Metric movement was already reported separately and is unaffected, on `metric_deltas`.
  - Null and empty fields are omitted from the structured channel rather than serialized as `null` / `[]` / `{}`, on `EvaluationResult`, `ProjectFileEntry`, and the models embedded in them (`RefactorTarget`, `SecurityFinding`, `AcknowledgedRisk`, `FunctionEntry`, `Suggestion`, `PreferenceWalk`, `AgentContract`). Consumers must treat an absent key as empty. The one-per-response wrappers (project, comparison, coverage, depgraph, refactor results) keep their full key set.
  - `ProjectEvaluationResult.worst_files` is now a compact `{filepath, lattice_element}` list instead of three complete `ProjectFileEntry` clones of the first page's rows. It is still the three globally worst files by the existing sort (ascending minimum score across measured dimensions) and is page-global — unaffected by `offset`/`limit`. Callers that read other fields off `worst_files` should read the corresponding `files` row instead.
- **`topos_evaluate_file(refactor_targets=…)` defaults to `3` instead of `0`.** Ranked edit targets — ordered gate failures first, since `cfg.cyclomatic`'s advisory failures no longer outrank real ones — now come back without a second round trip. Their expensive input was already computed on every call, so this costs nothing but the payload. `0` still disables them, `25` is still the cap, and the "re-run with refactor_targets" hint in `agent_contract.next_actions` now appears only for callers that explicitly opted out.
- **New `EvaluationResult.binding_constraint`.** The single gating metric costing a pillar its `achieved`: pillar, metric, value, threshold, and span. It is the top-ranked `"fix"` refactor target projected down to those fields, so it cannot disagree with `refactor_targets`, and it is omitted when only advisory metrics are out of band — which is what makes the common `achieved: true` / `score: 0.0` combination readable. The agent contract's edit step now derives from the top *gating* target rather than the first target, so an advisory-only result no longer opens an edit/assess loop over a metric no verdict depends on.
- **MCP server rewritten in Rust (`topos-mcp` crate)**: the entire `topos/mcp/**` Python package is reimplemented as a Rust `rmcp` stdio server, so no computation is marshalled through Python anymore — every tool (all 18: the `topos_evaluate_*`, `topos_assess_*`, `topos_compare_*`, `topos_inspect_code`, `topos_calculate_coverage`, `topos_preference_walk`, `topos_depgraph_*`, `topos_generate_*`, `topos_refactor`, `topos_get_doc` family), all 6 `topos://docs/*` resources, and the `topos_refactor_until_ideal` prompt call directly into `topos-core`. The `topos-mcp` PyPI package is now a thin maturin `bin` wheel that ships this self-contained server binary; `pip install topos-mcp` puts the `topos-mcp` command on `PATH` with zero Python runtime dependencies.
...
- **The legacy Python implementation is deleted**: `topos/` (the Python package — MCP, functors, graphs, core, evaluation, CLI, utils) and the entire Python `tests/` suite are gone now that computation lives in `topos-core` and the server is `topos-mcp`. Also removed: the PyInstaller onefile build (`scripts/build-binary.sh`, `scripts/lazy_exports.py`, `packaging/macos-entitlements.plist`), and the Sphinx `docs/source/api/` autodoc pages (which introspected the removed Python API — there's no Rust equivalent; see the new `docs/source/architecture.rst`). The rest of the Sphinx site (`docs.krv.ai`) stays and is rewritten for the new Rust CLI/MCP surface and crate structure — see `pyproject.toml`'s `docs` dependency group and `.github/workflows/docs.yml`. CI and the release workflow are rewritten Rust-only (cargo test/clippy/fmt + a stdio smoke test; binaries via `cargo build`, PyPI `bin` wheels via maturin).
- **Migration-time parity/performance spot-checks (not retained)**: during development, throwaway scripts diffed the new Rust `topos inspect --json` against the last published Python release (`topos-mcp==0.3.12`) over a small fixture corpus and timed a handful of invocations — an informal sanity check that the rewrite is a behavior-preserving, faster drop-in, not a rigorous benchmark. Those measurements were single-run and machine-specific, and the scripts and corpus were removed rather than kept as evergreen tooling. Formal parity and performance validation will be run against the shipped v0.4.0 release; treat any migration-era figures as indicative only.

### Fixed

- **`topos inspect --json` emits parseable JSON on the first run in a repository.** COMPOSABLE-by-default generates a missing `.gitnexus` before scoring, and that GitNexus subprocess inherited stdout to stream its progress banner live — landing inside the JSON document and making it unparseable. Only the first run was affected (a present graph skips generation), so the corruption hit exactly the fresh-clone path agents and parity scripts use. `inspect` now captures the child's output when `--json` is set; human-facing runs still stream progress, and all diagnostics remain on stderr.
- **The MCP agent docs and `topos_refactor_until_ideal` prompt taught a call shape the server rejects.** The Python FastMCP server wrapped each tool's arguments in a single `params` model; the Rust `rmcp` rewrite takes them at the top level, but every documented example kept the wrapper, so copying one returned `failed to deserialize parameters: unknown field 'params'`. All examples are now flat, and regression tests pin the docs to the registered router (no `params` wrapper, every tool documented, no doc naming an unregistered tool).
- **`ast.max_function_complexity` restores pre-migration decision-form counting via UAST** (closes [#142](https://github.com/Krv-Labs/topos/issues/142)): the Rust engine's language-neutral UAST walk now counts short-circuit boolean operators (`and`/`or`/`&&`/`||`), ternaries, Python comprehensions (per implicit `for` and filter `if`), `with`/`assert`, and per-handler `except`/`catch` arms — forms the Python→Rust migration dropped. Mappers populate `BinaryExpr`'s `"operator"` attribute; new canonical UAST kinds (`ConditionalExpr`, `Comprehension`, `WithStmt`, `AssertStmt`, `CatchClause`) are mapped per language where the construct exists. The SIMPLE gate threshold (`<= 10.0`) is unchanged; richer counting makes the gate stricter, not looser.
- **`cfg.cyclomatic` / `ast.max_function_complexity` now count `match`/`switch` in every language** (completes #151/#153): Python `match`, JavaScript/TypeScript `switch`, and C++ `switch` were never mapped to the UAST `MatchStmt` kind, so their case dispatch added nothing to complexity and a match-heavy function could pass the SIMPLE gate it should fail. Every language mapper now emits `MatchStmt`, and the CFG builder's arm extractor was unified so each case is exactly one branch — also fixing a discriminant-less Go `switch` over-count where each case *statement* became its own branch. Both metrics now count one branch per case arm, so `ast.max_function_complexity` agrees with `cfg.cyclomatic`; this intentionally diverges from `topos-mcp==0.3.12` (which counted a whole match/switch as a single decision).
